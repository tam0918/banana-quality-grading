...
Sau 50 epoch huấn luyện trên tập dữ liệu Kaggle Banana Classification \ref{ref:kaggle_banana_classification}, mô hình classifier đạt được kết quả ấn tượng:
...
Intel i7-10700 & CPU & 20 ms & 13 ms & \textbf{$\sim$33 ms} \\
Intel i5-8250U & CPU & 25 ms & 15 ms & \textbf{$\sim$40 ms} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{CPU mạnh (i7)}: 25-30 FPS (real-time tốt)
    \item \textbf{CPU phổ thông (i5)}: 25-30 FPS (real-time tốt)
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Hiệu năng trên CPU cũ}: Với CPU đời cũ hoặc laptop tiết kiệm pin, FPS có thể giảm. Tuy nhiên, trên các CPU Intel Core i5/i7 thế hệ gần đây, hệ thống vẫn đạt 25-30 FPS, đủ cho real-time.
...
Tốc độ xử lý (CPU) & Tốt (25-30 FPS) & 8/10 \\
...
\begin{enumerate}
    ...
    \item \textbf{Dataset chất lượng}: Banana Classification Dataset \ref{ref:kaggle_banana_classification} được annotation bởi Roboflow với tiêu chuẩn nhất quán, cân bằng giữa các class.
...
Mendoza \& Aguilera (2004) \ref{ref:banana_ripening} & 93\% & 7 & Phương pháp thống kê màu \\
Mazen \& Nashat (2019) \ref{ref:banana_ripeness} & 96.7\% & 3 & ANN, không real-time \\
